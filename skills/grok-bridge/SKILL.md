...
Report submitted, completed, blocked, or unverified status accurately, preserve the request/session identifier, and continue the parent task without a reciprocal loop. See the [acceptance record](https://github.com/niharnm/grok-bridge/blob/main/docs/acceptance.md) for current live coverage; do not infer all eight directions or native skill loading from a successful installation.
